"""Demo: act as an EXTERNAL agent plugging into AgentGuard over MCP.

Launches the AgentGuard MCP server (stdio) and, as a separate "host agent", submits three actions
...
The deploy lands as 'pending' — approve it on the dashboard (uvicorn agentguard.api:app) and the
...
# Make `agentguard` importable when run as a plain script.
...
async def main() -> None:
    ...
    env = {
        **os.environ,
        "AGENTGUARD_DB": db_path,
        "ANTHROPIC_API_KEY": "",
    }  # key-free: rules + fail-safe
    params = StdioServerParameters(
        command=sys.executable, args=["-m", "agentguard.mcp_server"], env=env
    )

    async with stdio_client(params) as (read, write), ClientSession(read, write) as session:
        await session.initialize()
        tools = [t.name for t in (await session.list_tools()).tools]
        print(f"connected to AgentGuard MCP server. tools: {tools}\n")
