...
- **Two-model comparison (planned, not yet generated).** Re-run the calibration with a second judge model (Haiku vs Sonnet) and overlay the curves — turns *"would a better model be more reliable?"* into a *shown* result. This is the concrete form of the model-tiering ablation; run at `temperature=0`, report mean ± spread across seeds. *Cost framework activates only at benchmark scale (Tier 3);* until then runs are pennies.
